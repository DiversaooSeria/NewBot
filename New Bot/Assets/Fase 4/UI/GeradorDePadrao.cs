...
public class GeradorDePadrao : MonoBehaviour
{
    ...
    public void InitializeInventoryUI(int inventorySize)
    {
        ...
        for (int i = 1; i < inventorySize; i++)
        {
            ...
            // Ajusta a posi��o e a escala corretamente para UI
            ...
            if (Enum.IsDefined(typeof(Formas), mecrec.gerente.sequenciaInicial[i - 1]) && mecrec.gerente.sequenciaInicial[i - 1] != 4)
            {
                ...
                if (prefabForma == null)
                {
                    Debug.LogWarning($"Forma '{nomeForma}' n�o encontrada em EntregaForma.");
                    return;
                }
                ...
                // Configura��o de UI para garantir posicionamento correto
                ...
                if (novaFormaRect != null)
                {
                    ...
                    novaForma.transform.SetAsLastSibling();       // Mant�m a ordem correta na UI
                }
                else
                {
                    Debug.LogWarning("Nova forma n�o possui RectTransform.");
                }

                // Configura��o do comportamento da forma
                Itens itemScript = novaForma.GetComponent<Itens>();
                if (itemScript != null)
                {
                    itemScript.parentBeforeDrag = uiItem.transform;
                    itemScript.locked = true; // O item n�o pode ser movido
                }
                else
                {
                    Debug.LogWarning("Nova forma n�o possui componente 'Itens'.");
                }
                ...
                {
                    Debug.LogWarning("uiItem n�o possui componente 'DropContainer'.");
                }
                else
                {
